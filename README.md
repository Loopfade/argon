...
Локально прошли 17 Python-тестов, включая выбор архитектуры и проверку ABI/метаданных на тестовых ZIP-файлах. Ранее прошли 32 проверки политики на закреплённом BoringSSL и проверка применения патча к исходникам Chromium после соответствующих патчей Vanadium. Это не заменяет сборку всего браузера и испытания каждого ABI на Android; подробности — в [VALIDATION.md](VALIDATION.md).
...
- Отдельные цели сборки для `arm64-v8a`, `armeabi-v7a`, `x86_64` и `x86`; версии Chromium, Vanadium, BoringSSL, depot_tools и предустановленного расширения зафиксированы в `build-lock.json`. Автоматического перехода на новые версии УЦ или движка нет.

## Архитектуры

| Android ABI | Аргумент `build.sh` / поле `arch` в Actions | Целевые процессоры |
|---|---|---|
| `arm64-v8a` | `arm64` (по умолчанию) | ARM 64 бит |
| `armeabi-v7a` | `arm` | ARM 32 бита с NEON |
| `x86_64` | `x64` | Intel/AMD 64 бита, в том числе эмуляторы |
| `x86` | `x86` | Intel 32 бита и эмуляторы |

Это текущий набор [поддерживаемых ABI Android NDK](https://developer.android.com/ndk/guides/abis). Командная строка также принимает имена ABI: например, `bash build.sh x86_64`. `armeabi`, MIPS и RISC-V не добавлены в список целей.

Для каждого ABI создаётся отдельный APK; библиотеки второго ABI не включаются. Настройка `is_desktop_android`, необходимая для расширений, сохраняется для всех целей. DrumBrake (интерпретатор WebAssembly) включён только для `arm64` и `x64`, поскольку закреплённый V8/Vanadium не поддерживает его на 32-битных целях. На `arm` и `x86` WebAssembly в режиме без JIT недоступен.
...
2. Откройте **Actions → Build Titanium RU → Run workflow** и выберите `arch` из таблицы выше. Значение `all` запускает четыре независимые сборки; по умолчанию остаётся одна сборка `arm64`.
...
5. Только после успешной сборки, проверки подписи, package, имени, выбранного ABI, ZIP и выравнивания APK появится в artifact `Titanium-RU-<arch>-<commit>`. Внутри — каталог `<ABI>/` с APK, SHA-256, открытым сертификатом подписи, лицензиями и `build-info.json`. Метаданные и сертификаты разных архитектур не перезаписывают друг друга. Ошибка одной цели в режиме `all` не отменяет остальные.

Сценарий требует как минимум **100 GiB свободного места на каждую сборку**. Для своей машины разумно выделить 150–200 GiB и 32 GiB RAM; нужен Ubuntu/Linux x64 с `sudo` для установки сборочных зависимостей. Требования Chromium: https://chromium.googlesource.com/chromium/src/+/main/docs/android_build_instructions.md . Полная сборка может занять несколько часов и расходует минуты GitHub Actions; `all` выполняет четыре такие сборки. Успех на стандартном runner пока не проверен.

Сборка из нового локального checkout:

```sh
git submodule update --init
SIGNING_MODE=test bash build.sh x64
```

Вместо `x64` можно выбрать любую цель из таблицы. `bash build.sh` без аргументов и старый `bash build-arm64.sh` собирают `arm64`. GN-файл создаётся в `chromium/src/out/TitaniumRu-<arch>/args.gn` из общего шаблона `args.gn`, результаты подписи — в `artifacts/<ABI>/`.

Повторный запуск в частично собранном дереве намеренно останавливается. Для каждой архитектуры используйте новый рабочий checkout или отдельную задачу Actions. Скрипт не удаляет существующий Chromium checkout автоматически и не меняет закреплённый submodule Vanadium.
...
```sh
for arch in arm64 arm x64 x86; do
  python3 scripts/preflight.py --inputs-only --arch "$arch"
done
...
Workflow `Validate scoped Russian CA` проверяет входные данные и генерацию GN для всех четырёх целей на обычном GitHub runner. Python-тесты также проверяют отказ при неверном/смешанном ABI, разделение метаданных и совместимость старого скрипта. Команды Android SDK/JDK в тесте упаковки имитируются: это не проверка подписи настоящего APK. Проверки C++ вызывают производственную реализацию ограничения через настоящий BoringSSL, а не повторяют её алгоритм на другом языке.

После получения APK необходимо проверить на устройстве или эмуляторе соответствующей архитектуры: запуск приложения, страницу с российской цепочкой в разрешённой зоне, обычные HTTPS-сайты, установку и работу расширения, инкогнито и поведение при неверных сертификатах. Эти испытания пока не выполнены ни для одной цели.
