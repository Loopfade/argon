...
Сборка доступна [локально на Debian](BUILDING.md) или через **Actions → Build Argon → Run workflow**. В Actions значение `arch=all` собирает все архитектуры; локально `build.sh` собирает по одной ABI. Для каждой сборки требуется не менее 100 GiB свободного места.
