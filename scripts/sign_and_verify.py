#!/usr/bin/env python3
"""Sign and verify the arm64 APK, then write checksums and build metadata."""
import argparse
import base64
import hashlib
import json
...
def main():
    ...
    name = f'Titanium-RU-{lock["chromium_version"]}-{args.mode}-arm64-v8a.apk'
    ...
    with zipfile.ZipFile(output) as archive:
        ...
        if abis != {"arm64-v8a"}:
            raise SystemExit(f"Unexpected APK native ABIs: {sorted(abis)}")
