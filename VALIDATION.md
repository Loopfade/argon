# Проверки сборки и подготовленных исходников

## Состояние на 2026-09-24

- Все коммиты `experimental/ca-domain-allowlist` вошли в `main` через merge-коммит `5711efb2a9d51bd89efcb52b28ff494d05a79fca`.
- [Release-сборка ARM64](https://github.com/Loopfade/argon/actions/runs/35946576052) успешна. Исходный commit APK: `0d8cd244926a63690ff060a025d1959eb1c2780e`. Подпись APK проверена сборщиком: APK Signature Scheme v3, RSA 4096, сертификат `CN=Argon Release, O=Argon`.
- [Опубликованный APK](https://github.com/Loopfade/argon/releases/tag/v153.0.8010.52-argon.1) повторно сверён с исходным Actions artifact: 324507446 байт, SHA-256 `042fdc152edcdf831d71cefe4f440eb921ab2d95f3d04cf734ed28f65bcea0de`. Неполная первая загрузка заменена; GitHub подтверждает этот размер и digest.
- Владелец проекта подтвердил работу APK на Android. Отдельное прохождение всех отрицательных TLS/UI-сценариев этим сообщением не подтверждается.
- 55 Python-тестов прошли при повторной локальной проверке, включая настоящий закреплённый фильтр без доступа к сети. Остальные ABI не собирались: один запуск `Build Argon` собирает одну архитектуру, `arch=all` не поддерживается.
- Текущий CI сохраняет `ccache` до 20 раз с интервалом 20 минут, с учётом шестичасового лимита и резервов на завершение. Линковка не сохраняется в `ccache`.
- Кэш GitHub Actions привязан к ветке: merge не переносит его в `main`. Для переноса предусмотрен ручной `Transfer compiler cache`: сначала `export` в исходной ветке, затем `import` в `main` с ID успешного экспорта. Исходную ветку следует удалять после подтверждения импорта.
- Кэш успешной ARM64-сборки перенесён: [экспорт](https://github.com/Loopfade/argon/actions/runs/35976536899), [проверенный импорт в main](https://github.com/Loopfade/argon/actions/runs/35977013345). Контрольная сумма архива совпала; новый кэш подтверждён через API перед удалением старых снимков `main`.
- При холодной сборке образа закреплённый `antiadblockfilters.txt` берётся из `docker/chromium/filter-cache/` и повторно проверяется по SHA-256. Снимок извлечён из проверенного immutable-образа; новые байты изменяемого upstream URL не подменяют закреплённый файл, второй большой образ для сборки не нужен.

Ниже сохранён журнал более ранних проверок на 2026-09-21. Его ограничения на отсутствие готового APK относятся к состоянию на ту дату.

## Пользовательский список дополнительных доменов УЦ
...
| Python regression/idempotence tests | 29 тестов пройдено локально |
...
| GitHub Actions | Сторонние Actions закреплены commit SHA; `Build Argon` прогревает общий ограниченный `ccache` контрольными точками и собирает одну ABI за запуск. Успешная release-сборка arm64: [run 35946576052](https://github.com/Loopfade/argon/actions/runs/35946576052) |
...
собирает BoringSSL test, а workflow `Build Argon` компилирует production APK.
Chromium gtest сохранён в исходниках, но его Android binary не входит в APK workflow:
холодная сборка одного `chrome_public_apk` уже превышает предел hosted job. Запуск
Android gtest и UI/TLS smoke-тестов всё ещё требует устройства или эмулятора.
...
| Python-тесты | 29 пройдено |
| Синтаксис shell/Python/YAML и `git diff --check` | Пройдены |
| Возобновляемая CI-сборка | GN переключается с локального Siso на Ninja с `ccache`; два четырёхчасовых warm-up job передают только сжатый compiler cache, финальный job выполняет cache hits, линковку, подпись и проверку APK |

На эту дату полная сборка Chromium, GN generation в полном дереве и запуск новых APK здесь не были выполнены: доступны около 29 GiB вместо требуемых 100 GiB, нет Android SDK и полного checkout зависимостей. Тесты упаковки не подтверждают действительность подписи реального APK. Для полной проверки каждой архитектуры используйте отдельный ручной workflow `Build Argon` с одним из значений `arm64`, `arm`, `x64`, `x86`, затем испытания на соответствующем устройстве/эмуляторе.
...
| Холодная сборка на стандартном runner | [Запуск №3](https://github.com/Loopfade/argon/actions/runs/35432031468) отменён жёстким лимитом 6 часов: `chrome_public_apk` компилировался 5 ч 42 мин и оставлял около 80 тысяч операций. Увеличение `timeout-minutes` не помогает |
