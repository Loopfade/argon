...
- 54 Python-теста прошли при повторной локальной проверке `main`. Остальные ABI не собирались: один запуск `Build Argon` собирает одну архитектуру, `arch=all` не поддерживается.
...
- При холодной сборке образа закреплённый `antiadblockfilters.txt` берётся из проверенного immutable-образа и повторно проверяется по SHA-256; новые байты изменяемого upstream URL не подменяют закреплённый файл.
